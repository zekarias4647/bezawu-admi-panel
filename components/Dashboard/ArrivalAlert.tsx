...
const ArrivalAlert: React.FC<ArrivalAlertProps> = ({ order, onClose, onComplete, isDarkMode }) => {
  return (
    <div className="fixed inset-0 z-[9999] flex items-center justify-center p-4 bg-black/15 backdrop-blur-sm animate-in fade-in duration-300">
      <div className={`max-w-[540px] w-full rounded-[2.5rem] shadow-2xl transition-all border animate-in zoom-in-95 duration-200 overflow-hidden relative ${
        ...
        <div className="px-12 pb-12">
          <div className={`rounded-[2rem] border transition-colors p-8 space-y-8 ${
            isDarkMode ? 'bg-[#0f1115] border-slate-800' : 'bg-[#f8fafc] border-slate-200'
          }`}>
            ...
            <div className={`p-5 rounded-2xl border flex items-center gap-3 transition-colors ${
          ...
          <div className="grid grid-cols-2 gap-6 mt-10">
            <button 
              onClick={onClose}
              className={`py-5 rounded-2xl font-bold text-sm tracking-widest uppercase transition-all ${
                isDarkMode ? 'bg-slate-800 text-slate-400 hover:bg-slate-700' : 'bg-[#e2e8f0] text-[#475569] hover:bg-[#cbd5e1]'
              }`}
            >
              NOT NOW
            </button>
