...
const Sidebar: React.FC<SidebarProps> = ({ user, activeTab, onTabChange, onLogout, isDarkMode }) => {
  return (
    <aside className={`w-72 border-r flex flex-col transition-colors duration-300 ${isDarkMode ? 'bg-[#121418] border-slate-800' : 'bg-white border-slate-200'
      }`}>
      <div className="p-8 flex items-center gap-3">
        ...
        <div>
          <h1 className={`text-xl font-black tracking-tighter ${isDarkMode ? 'text-white' : 'text-slate-900'}`}>{user.supermarketName.toUpperCase()}</h1>
      ...
      <nav className="flex-1 px-4 space-y-2">
        ...
        {NAVIGATION.map((item) => (
          <button
            ...
            className={`w-full flex items-center gap-4 px-5 py-4 rounded-[1.2rem] transition-all group border ${activeTab === item.id
      ...
      <div className="p-6">
        <button
          onClick={onLogout}
          className={`w-full flex items-center gap-4 px-5 py-4 rounded-[1.2rem] transition-all border border-transparent ${isDarkMode ? 'text-slate-600 hover:text-rose-400 hover:bg-rose-400/10' : 'text-slate-400 hover:text-rose-600 hover:bg-rose-50'
            }`}
        >
          <LogOut size={20} />
